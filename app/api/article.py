import inject
from flask import Blueprint, render_template, url_for, redirect, request, jsonify, current_app, flash

from app.service import CommentService, PostsService, CommentLikeService
...
comment_service = inject.instance(CommentService)
commentLike_service = inject.instance(CommentLikeService)
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            ...
            p = Posts(title=form.title.data, describe=form.content.data[0:145] + '……', content=form.content.data,
                      user=u)
            # 添加进入数据库
            posts_service.create_article(p)
...
def mypublish():
    ...
    pagination = posts_service.pagination(page, per_page=5, uid=u.id)  # 获取分页
...
def article_detail():
    ...
    posts = posts_service.article_detail(id=id)  # 获取文章详情
    try:
        comments = comment_service.get_comment(post_id=id)  # 获取文章评论
    ...
    try:
        ...
        commentlikes = []
        if current_user.is_authenticated:
            commentlikes = commentLike_service.get_commentLike(
                user_id=current_user._get_current_object().id)  # 获取评论点赞信息
        # 6.1.2获取用户所有点赞过的评论编号
        mylike_comment_ids = []
...
def article_delete():
    ...
    try:
        posts_service.delete_article(id=id)
    except:
        flash("删除失败")
...
def article_change():
    ...
    if request.method == 'GET':
        ...
        posts = posts_service.article_detail(id=id)
        ...
        if form.validate_on_submit():
            p = posts_service.article_detail(id=form.id.data)
            p.title = form.title.data
            p.content = form.content.data
            p.describe = form.content.data[0:145] + '……'
            p.timestamp = datetime.utcnow()
            posts_service.update_article(id=form.id.data, entity=p.update_dict())
            return redirect(url_for("article.mypublish"))
...
def article_search():
    ...
    pagination = posts_service.search_pagination(keyword, page, per_page=5)  # 获取搜索信息
    posts = pagination.items
    return render_template('article/search.html', posts=posts, pagination=pagination, keyword=keyword)


# 文章评论后端
# 请求路径: /article/article_comment
# 请求方式: POST
...
def article_comment():
    ...
    # 4. 根据文章编号取出文章对象,判断文章是否存在
    try:
        post = posts_service.article_detail(id=post_id)
    except Exception as e:
        current_app.logger.error(e)
    ...
    # 6. 保存评论对象到数据库中
    try:
        comment_service.create_comment(comment)  # 创建新的评论对象
    except Exception as e:
        current_app.logger.error(e)
...
def comment_like():
    ...
    # 5. 通过评论编号查询评论对象,并判断是否存在
    try:
        comment = comment_service.get_comment_by_id(id=comment_id)
    except Exception as e:
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR, errmsg="获取评论失败")
    ...
    try:
        if action == "add":
            ...
            comment_like = commentLike_service.get_commentLike(user_id=current_user._get_current_object().id,
            ...
            if not comment_like:
                ...
                # 添加到数据库中
                commentLike_service.create_commentLike(comment_like)
        else:
            # 6.2 判断用户是否有对该评论点过赞
            # comment_like = CommentLike.query.filter(CommentLike.user_id == current_user._get_current_object().id,
            #                                         CommentLike.comment_id == comment_id).first()
            ...
            if comment_like:
                ...
                if comment.like_count > 0:
                    comment.like_count -= 1
                commentLike_service.delete_commentLike(user_id=current_user._get_current_object().id,
                                                       comment_id=comment_id)
                comment_service.update_commnet(id=comment.id,entity=comment.update_dict())
    except Exception as e:
        current_app.logger.error(e)
